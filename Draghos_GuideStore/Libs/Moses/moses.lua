...
--- Returns the first value passing an iterator test.
-- @name findWith
-- @param t a table
-- @param f an iterator function, prototyped as `f (v, k)`
-- @return a value from the passed-in table
-- @see findWhere
function M.findWith(t, f)
    for index,value in pairs(t) do
      if f(value,index) then return value end
    end
end
...
--- Replaces elements with the given keys in an array with the provided value. In case an element does not exist for one
-- of the keys it is created.
-- @name fillWith
-- @param array an array
...
-- @return the original array with values changed
-- @see fill
function M.fillWith(array, keys, value)
...
function M.flatten(array, shallow)
  ...
  for key,value in pairs(array) do
    if type(value) == 'table' then
      ...
      for k,item in pairs(new_flattened) do _flat[#_flat+1] = item end
...
-- @name unique
-- @param array an array
-- @param[opt] find a function that looks for the occurence of a value in an array (defaults to `M.find`)
-- @return a new array, duplicate-free
-- @see isunique
-- @see duplicates
function M.unique(array, find)
  find = find or M.find
  local ret = {}
  for i = 1, #array do
...
--- Produces an array with duplicate*-free value on a given prop.
-- <br/><em>Aliased as `uniq`</em>
-- @name unique
-- @param array an array
-- @param prop the name of the property that must have a unique value in the array
-- @return a new array, duplicate-free
-- @see unique
function M.uniqueProp(array, prop)
  return M.unique(array, function(ret, item)
    return M.findWhere(ret, {[prop] = item[prop]})
  end)
end
...
do
  ...
  local MAJOR, MINOR = "Moses", 1;
  assert(LibStub, MAJOR .. " requires LibStub");
  local Moses = LibStub:NewLibrary(MAJOR, MINOR);
  Moses.__index = f
  ...
  -- @name import
  -- @param[opt] context a context. Defaults to `_G` (current environment).
